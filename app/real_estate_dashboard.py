...
# ________________________________________________________________________
# Load DB (loading data from the DB) and the ML model used to predict prices
...
# VERY IMPORTANT: get column names used by the model
...
#   ________________________________________________________________________
# New design, adding tabs to integrate predictive analysis using the best model
tab1, tab2 = st.tabs(["📊 Market Overview", "❓ Rent Price Predictor"])
...
with tab2:
    ...
    st.caption("Estimate fair annual rent based on unit characteristics and location.")

    # USER INPUTS
    col1, col2 = st.columns(2)

    with col1:
        bedrooms = st.selectbox("Bedrooms", sorted(df["bedrooms_clean"].unique()))
        bathrooms = st.selectbox("Bathrooms", sorted(df["bathrooms_clean"].unique()))
        area = st.number_input("Apartment Size (sqft)", min_value=19, max_value=9999, step=50)

    with col2:
        building_map = {
            c.replace("building_grouped_", "").replace("_", " "): c
            for c in model_columns if c.startswith("building_grouped_")
        }
        district_map = {
            c.replace("district_", "").replace("_", " "): c
            for c in model_columns if c.startswith("district_")
        }

        building = st.selectbox("Building", building_map)
        district = st.selectbox("District", district_map)
    ...
    if st.button("Predict Rent"):
        ...
        st.success(f" Estimated Annual Rent: {prediction:,.0f} AED")

        # Market comparison
        market_avg = df["price_yearly_aed"].mean()
        diff_pct = ((prediction - market_avg) / market_avg) * 100


        st.info(
            f"📊 Compared to the overall JVC average ({market_avg:,.0f} AED), "
            f"this unit is **{diff_pct:+.1f}%**."
        )
